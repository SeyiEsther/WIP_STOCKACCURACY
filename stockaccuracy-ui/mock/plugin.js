import { comparison, summary, trend, materialTrends } from './data.js'
...
export function mockApiPlugin() {
  return {
    ...
    configureServer(server) {
      ...
      server.middlewares.use('/api/stock/material-trends', (req, res) => {
        // days param is ignored in mock — just return all precomputed trends
        res.setHeader('Content-Type', 'application/json')
        res.end(JSON.stringify(materialTrends))
      })
      ...
      server.middlewares.use('/api/stock/export', (req, res) => {
        ...
        const header = 'materialNumber,materialDesc,sLoc,qtyYesterday,qtyToday,delta,pctChange,status,baseUnit,mrpController,unitValue,todayDate,yesterdayDate'
        const lines = rows.map(r =>
          [r.materialNumber, `"${r.materialDesc}"`, r.sLoc,
           r.qtyYesterday, r.qtyToday, r.delta, r.pctChange,
           r.status, r.baseUnit, r.mrpController,
           r.unitValue ?? '',
        ...
        res.setHeader('Content-Disposition',
          `attachment; filename="stock-accuracy-${new Date().toISOString().slice(0, 10)}.csv"`)
        res.end(csv)
      })
