// ─── PaintedWatchlistPage.jsx — the "Painted Parts" tab ───────────────────────
// A focused view of the painted (e.g. RAL9005) materials we watch, showing the
// current quantity of each. Like the Watchlist tab, this page fetches its own
// data — from /api/watchlist/painted — rather than receiving it from App.jsx,
// and refreshes it once an hour on a timer.

import { useState, useEffect, useCallback, useRef } from 'react'

const API_URL = '/api/watchlist/painted'
const REFRESH_MS = 60 * 60 * 1000   // auto-refresh every 60 minutes

// Format a number with thousands separators; show "—" when there's no value.
const fmtNum = (n) => {
  if (n == null) return '—'
...
// The API may return field names in PascalCase or camelCase, so smooth each row
// into a single tidy shape (same idea as norm() in lib/normalize.js).
function normRow(r) {
  return {
...
const COLS = [
  ...
  { key: 'qty',         label: 'Qty',              align: 'right', width: 120 },
]

export default function PaintedWatchlistPage() {
        ...
        <table style={{ width: '100%', minWidth: 480, borderCollapse: 'collapse', tableLayout: 'fixed' }}>
          ...
          <tbody>
            {loading && rows.length === 0 ? (
              ...
              <tr><td colSpan={COLS.length} style={EMPTY_CELL}>No painted parts on the watchlist.</td></tr>
            ) : (
              rows.map((r, i) => (
